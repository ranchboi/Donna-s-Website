...
// Load blocked times from localStorage
function loadBlockedTimesFromStorage() {
    const storedBlockedTimes = localStorage.getItem('blockedTimes');
    if (storedBlockedTimes) {
        blockedTimes = JSON.parse(storedBlockedTimes);
        console.log('Loaded blocked times from storage:', blockedTimes);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadBlockedTimesFromStorage(); // Load blocked times on startup
    setupLogin();
    updateStats();
...
function generateAdminCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (currentDate < today.setHours(0, 0, 0, 0)) {
            ...
            dayElement.title = 'Blocked - Click to unblock time slots';
        ...
        dayElement.addEventListener('click', () => {
            if (!dayElement.classList.contains('disabled')) {
                if (blocked) {
                    // Open multi-select unblock modal for blocked dates
                    showMultiUnblockModal(dateStr);
                } else {
                    // Show regular day details for non-blocked dates
...
function showBlockedTimesModal() {
    loadBlockedTimes();
    document.getElementById('blockedTimesModal').style.display = 'block';
}

function closeBlockedTimesModal() {
    document.getElementById('blockedTimesModal').style.display = 'none';
}

function loadBlockedTimes() {
    const blockedTimesList = document.getElementById('blockedTimesList');
    const noBlockedTimes = document.getElementById('noBlockedTimes');
    
    // Get blocked times from localStorage (or use the default array)
    const storedBlockedTimes = localStorage.getItem('blockedTimes');
    if (storedBlockedTimes) {
        blockedTimes = JSON.parse(storedBlockedTimes);
    }
    ...
    if (blockedTimes.length === 0) {
        blockedTimesList.style.display = 'none';
        noBlockedTimes.style.display = 'block';
        return;
    }
    
    blockedTimesList.style.display = 'block';
    noBlockedTimes.style.display = 'none';
    
    // Sort blocked times by date
    const sortedBlockedTimes = blockedTimes.sort((a, b) => new Date(a.date) - new Date(b.date));
    
    blockedTimesList.innerHTML = sortedBlockedTimes.map((blockedTime, index) => {
        const date = new Date(blockedTime.date);
        ...
        const startTime = formatTime(blockedTime.startTime);
        const endTime = formatTime(blockedTime.endTime);
        
        return `
            <div class="blocked-time-item">
                <div class="blocked-time-info">
                    <h4 class="blocked-time-date">${formattedDate}</h4>
                    <p class="blocked-time-duration">${startTime} - ${endTime}</p>
                    ${blockedTime.reason ? `<p><strong>Reason:</strong> ${blockedTime.reason}</p>` : ''}
                </div>
                <div class="blocked-time-actions">
                    <button class="btn-unblock" onclick="unblockTime(${index})">
...
function unblockTime(index) {
    ...
    const confirmation = confirm(
        ...
        `${blockedTime.reason ? `Reason: ${blockedTime.reason}` : ''}`
    );
    
    if (confirmation) {
        // Remove the blocked time from the array
        blockedTimes.splice(index, 1);
        
        // Save to localStorage
        localStorage.setItem('blockedTimes', JSON.stringify(blockedTimes));
        
        // Refresh the blocked times list
        loadBlockedTimes();
        
        // Refresh the calendar and schedule to reflect changes
        generateAdminCalendar();
        loadTodaySchedule();
...
// Multi-Select Unblock Modal Functions
function showMultiUnblockModal(dateStr) {
    // Get blocked time slots for this specific date
    const dateBlockedTimes = blockedTimes.filter(block => block.date === dateStr);
    
    if (dateBlockedTimes.length === 0) {
        // No blocked times for this date
        document.getElementById('blockedSlotsToUnblock').style.display = 'none';
        document.getElementById('noBlockedSlotsOnDate').style.display = 'block';
    } else {
        document.getElementById('blockedSlotsToUnblock').style.display = 'block';
        document.getElementById('noBlockedSlotsOnDate').style.display = 'none';
        
        // Populate the blocked slots checkboxes
        loadBlockedSlotsForDate(dateStr, dateBlockedTimes);
    }
    ...
    document.getElementById('selectedDateInfo').innerHTML = `
        <div class="selected-date-info">
            ...
            <div class="selected-date-subtitle">Select time slots to unblock</div>
        </div>
    `;
    
    // Reset button state
    document.getElementById('unblockSelectedBtn').disabled = true;
    
    // Show modal
    document.getElementById('multiUnblockModal').style.display = 'block';
}

function loadBlockedSlotsForDate(dateStr, dateBlockedTimes) {
    const container = document.getElementById('blockedSlotsToUnblock');
    
    container.innerHTML = dateBlockedTimes.map((blockedTime, index) => {
        const originalIndex = blockedTimes.findIndex(bt => 
            bt.date === blockedTime.date && 
            bt.startTime === blockedTime.startTime && 
            bt.endTime === blockedTime.endTime
        );
        ...
        return `
            <div class="blocked-slot-checkbox" onclick="toggleSlotSelection(this)">
                <input type="checkbox" value="${originalIndex}" onchange="updateUnblockButton()">
                <div class="blocked-slot-info">
                    <div class="blocked-slot-time">${startTime} - ${endTime}</div>
                    ${blockedTime.reason ? `<div class="blocked-slot-reason">${blockedTime.reason}</div>` : ''}
                </div>
            </div>
        `;
    }).join('');
...
function updateUnblockButton() {
    const selectedCheckboxes = document.querySelectorAll('#blockedSlotsToUnblock input[type="checkbox"]:checked');
    const unblockBtn = document.getElementById('unblockSelectedBtn');
    
    if (selectedCheckboxes.length > 0) {
        unblockBtn.disabled = false;
        unblockBtn.innerHTML = `<i class="fas fa-unlock"></i> Unblock Selected (${selectedCheckboxes.length})`;
    } else {
        unblockBtn.disabled = true;
        unblockBtn.innerHTML = '<i class="fas fa-unlock"></i> Unblock Selected';
    }
}
...
function unblockSelectedSlots() {
    ...
    if (selectedIndices.length === 0) {
        alert('Please select at least one time slot to unblock.');
        return;
    }
    
    const confirmation = confirm(
        `Are you sure you want to unblock ${selectedIndices.length} time slot${selectedIndices.length > 1 ? 's' : ''}?`
    );
    
    if (confirmation) {
        // Sort indices in descending order to remove from end first (to maintain indices)
        selectedIndices.sort((a, b) => b - a);
        
        // Remove the blocked times
        ...
        // Close modal
        closeMultiUnblockModal();
        
        // Refresh the calendar and schedule
        generateAdminCalendar();
        loadTodaySchedule();
        
        alert(`${selectedIndices.length} time slot${selectedIndices.length > 1 ? 's have' : ' has'} been unblocked successfully!`);
    }
}

function closeMultiUnblockModal() {
    document.getElementById('multiUnblockModal').style.display = 'none';
}
...
function isTimeBlocked(date, time) {
    const blocked = blockedTimes.find(block => {
...
function convertTimeToHour(timeStr) {
    // Handle 24-hour format (e.g., "11:30", "12:00")
    if (timeStr.includes(':') && !timeStr.includes('AM') && !timeStr.includes('PM')) {
        const [hours, minutes] = timeStr.split(':').map(Number);
        return hours + (minutes / 60);
    }
    
    // Handle 12-hour format (e.g., "11:30 AM", "12:00 PM")
